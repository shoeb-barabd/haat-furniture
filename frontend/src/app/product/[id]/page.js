...
export default function SingleProductPage() {
  ...
  const buildUpdatedCart = () => {
    ...
    const foundIdx = next.findIndex((item) => item.id === product.id);
    if (foundIdx > -1) {
      next[foundIdx] = { ...next[foundIdx], quantity: next[foundIdx].quantity + quantity };
    } else {
      next.push(cartItem);
    }
    return next;
  ...
  const handleAddToCart = () => {
    try {
      persistCart(buildUpdatedCart());
      setIsCartOpen(true);
      showToastMsg(`${quantity} × "${product.name}" কার্টে যোগ হয়েছে`);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#fbf9f5] text-slate-800 font-sans antialiased">
      ...
      <div className="bg-[#f8f6f0] text-xs py-3 px-6 border-b border-[#e4d8c4] flex items-center justify-between gap-3">
        <Link href="/" className="font-extrabold text-[#a07c32] hover:underline flex items-center gap-1">
          <span>← Storefront / স্টোরে ফিরে যান</span>
        </Link>
        <div className="flex items-center gap-3">
          <span className="text-[#8a6a3a] hidden sm:inline">📞 Hotline: +8809617333990</span>
          <button
            type="button"
            onClick={() => setIsCartOpen(true)}
            className="relative h-9 px-3 rounded-full bg-white border border-[#e4d8c4] text-slate-800 text-xs font-bold flex items-center gap-1.5 shadow-sm"
          >
            <span>🛒</span>
            <span>Cart</span>
            ...
            <div className="md:col-span-6 space-y-4">
              ...
              <div className="pt-4 flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="px-8 py-3.5 rounded-xl bg-[#c59b27] hover:bg-[#a07c32] text-white font-extrabold text-xs uppercase tracking-wider shadow-lg shadow-[#c59b27]/25 transition-all hover:scale-102 active:scale-95 flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="px-6 py-3.5 rounded-xl bg-[#1a110d] hover:bg-[#3a2a1c] text-[#e6c875] font-extrabold text-xs uppercase tracking-wider border border-[#c59b27]/40"
                >
                  Buy Now / অর্ডার করুন
                </button>
              </div>
              <p className="text-[11px] text-[#6b5740] font-medium">
                Add to Cart চাপলে কার্ট খুলবে। সেখান থেকে Checkout করে অর্ডার করতে পারবেন।
              </p>
          ...
          <div className="p-6 sm:p-8 text-xs leading-relaxed text-slate-700">
            {activeTab === 'description' && (
              <div className="space-y-4">
                ...
                <p><strong>Guarantee:</strong> 5 Years free service warranty for manufacturing fault.</p>
            ...
            {activeTab === 'additional' && (
              <div className="space-y-2">
                ...
                <p><strong>Warranty Card:</strong> 5-Year Service Warranty Card Included (bring voucher + card for service).</p>
              </div>
            )}
            ...
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-[#e4d8c4]">
                <div>
                  ...
                  <p className="text-[11px] text-[#6b5740] mt-0.5">{cartCount} item{cartCount === 1 ? '' : 's'} added</p>
                </div>
              ...
              {cartItems.length === 0 ? (
                ...
                <div className="mt-5 space-y-3 max-h-[58vh] overflow-y-auto pr-1">
                  {cartItems.map((item) => (
                    <div key={item.id} className="p-3 rounded-2xl bg-[#fbf9f5] border border-[#e4d8c4] flex items-center gap-3">
                      <img src={item.image} alt={item.name} className="w-16 h-16 rounded-xl object-contain bg-white border border-[#e4d8c4] p-1" />
                      <div className="flex-1 min-w-0">
                        <h4 className="text-xs font-bold text-slate-900 truncate">{item.name}</h4>
                        <p className="text-xs text-emerald-700 font-bold mt-0.5">৳ {item.price?.toLocaleString()} BDT</p>
                        <div className="flex items-center gap-2 mt-2">
                          <button type="button" onClick={() => updateCartQty(item.id, -1)} className="w-6 h-6 rounded bg-white border border-[#e4d8c4] text-xs font-bold">-</button>
            ...
            {cartItems.length > 0 && (
              <div className="pt-4 border-t border-[#e4d8c4] space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-slate-900 font-black">৳ {cartTotal.toLocaleString()} BDT</span>
                </div>
                <Link
                  href="/checkout"
                  className="w-full py-4 rounded-2xl bg-[#c59b27] hover:bg-[#b48a1e] text-white text-xs font-black transition-all text-center block uppercase tracking-wider"
                >
                  Proceed to Checkout / অর্ডার করুন
                </Link>
                <a
                  href={`https://wa.me/8809617333990?text=${encodeURIComponent(`Hello Haat Furniture! I want to order:\n\n${cartItems.map((i) => `- ${i.name} (Qty: ${i.quantity}) - ৳${i.price * i.quantity}`).join('\n')}\n\nTotal: ৳${cartTotal.toLocaleString()} BDT`)}`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-full py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black text-center block"
                >
